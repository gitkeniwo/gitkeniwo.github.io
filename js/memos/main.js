...
if (memoDom) {
    ...
    btn.addEventListener("click", function () {
        btn.textContent = 'Continue loading more murmurs. Ok to proceed? (y/n)';
        updateHTMl(nextDom)
...
function updateHTMl(data) {
    ...
    // 解析 TAG 标签，添加样式
    // const TAG_REG = /#([^\s#]+?) /g;
    const TAG_REG = /#([^\s#]+?)(?=\s|$)/g; // 匹配后面是任意空白字符或字符串结尾
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        // 查找memos的id
        // console.log('正在处理第 ' + (i + 1) + ' 条 Memos 数据，ID: ' + data[i].name);
        ...
        if (memo.APIVersion === 'new') {
                    ...
                    if (resType === 'image') {
                        if (resexlink) {
                            imgUrl += '<div class="resimg">' +
                                '<img loading="lazy" alt="" src="' + resexlink + '"/>' +
                                '</div>';
                        } else {
                            resLink = memos + '/file/' + name + '/' + filename;
                            imgUrl += '<div class="resimg">' +
                                '<img loading="lazy" alt="" src="' + resLink + '"/>' +
                                '</div>';
                        }
                    } else {
                        resLink = memos + '/file/' + name + '/' + filename;
            ...
            if (data[i].resourceList && data[i].resourceList.length > 0) {
                ...
                for (var j = 0; j < resourceList.length; j++) {
                    ...
                    if (resType == 'image') {
                        imgUrl += '<div class="resimg"><img loading="lazy" alt="" src="' + resLink + '"/></div>'
...
if (memo.total === true) {
    // This script is injected lazily (on scroll), so the window 'load' event
    // has usually already fired — call directly when the doc is ready.
...
// Theme switching lives in js/theme-switcher.js (data-theme attribute on
// <html>, "cactus-theme" storage key). The dark mode block that used to sit
// here bound the same .theme-toggle button to a rival body-class scheme.
